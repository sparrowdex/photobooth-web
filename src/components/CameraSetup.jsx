...
//   );
// }
// smaller screen compatibility for camer doesnt work
...
export default function CameraSetup({ layout, onBack, onDone }) {
  ...
  const [countdown, setCountdown] = useState(3);
  const [showInstructions, setShowInstructions] = useState(false);
  // Always capture at 640x480 for overlays/strip layout
  const CAPTURE_WIDTH = 640;
  const CAPTURE_HEIGHT = 480;
  // Only the preview window is responsive
  const [previewSize, setPreviewSize] = useState({ width: CAPTURE_WIDTH, height: CAPTURE_HEIGHT });
  const videoRef = useRef(null);
  const countdownInterval = useRef(null);

  // Responsive camera preview sizing (not the capture size!)
  useEffect(() => {
    function updatePreviewSize() {
      const vw = Math.min(window.innerWidth, document.documentElement.clientWidth);
      const isMobile = vw < 700;
      const width = isMobile ? Math.min(vw * 0.96, CAPTURE_WIDTH) : CAPTURE_WIDTH;
      const height = Math.round((width / 4) * 3);
  ...
  // p5 sketch function (preview only)
  const sketch = (p5) => {
    let capture;
    p5.setup = () => {
      p5.createCanvas(previewSize.width, previewSize.height);
      capture = p5.createCapture(p5.VIDEO);
      capture.size(previewSize.width, previewSize.height);
      capture.hide();
      videoRef.current = capture;
    };
    p5.draw = () => {
      ...
      if (capture) {
        ...
        if (step === "countdown") {
          p5.fill(255, 0, 0, 180);
          p5.noStroke();
          p5.ellipse(previewSize.width / 2, previewSize.height / 2, 120, 120);
          p5.fill(255);
          p5.textSize(64);
          p5.textAlign(p5.CENTER, p5.CENTER);
          p5.text(countdown, previewSize.width / 2, previewSize.height / 2);
        }
      }
    };
  };
  ...
  // Capture photo and advance (always at CAPTURE_WIDTH x CAPTURE_HEIGHT)
  const capturePhoto = () => {
    setStep("capture");
    const canvas = document.createElement("canvas");
    ...
    const ctx = canvas.getContext("2d");
    // Always draw from the video at the original capture size
    ctx.drawImage(
      videoRef.current.elt,
      0, 0, videoRef.current.width, videoRef.current.height,
